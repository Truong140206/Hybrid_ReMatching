...
\begin{table}
\caption{Twenty-one cells: four datasets, the five pre-training configurations
of the original work, plus MAE on ImageNet-R. Seed 42; baseline is HRM-PET
($w=1$, $\beta=0$), proposed is $w=0.7$, $\beta=0.5$. 5-Datasets has five tasks,
the other three have ten. The baseline column shows how far apart the cells are.}
\label{tab:grid}
\begin{tabular}{lrrr}
\hline
Backbone & Baseline Acc@1 & $\Delta$ Acc@task & $\Delta$ Acc@1\\
\hline
\multicolumn{4}{l}{\emph{Split-ImageNet-R}}\\
Sup-21K   & 74.02 & $+2.18$ & $+1.49$\\
MoCo v3-1K& 68.47 & $+2.95$ & $+1.89$\\
iBOT-1K   & 71.89 & $+3.05$ & $+2.52$\\
iBOT-21K  & 73.29 & $+2.78$ & $+2.55$\\
DINO-1K   & 70.23 & $+2.93$ & $+2.68$\\
\emph{MAE-1K} & 40.20 & $+3.95$ & $+4.50$\\
\multicolumn{4}{l}{\emph{Split-CIFAR100}}\\
Sup-21K   & 89.77 & $+1.02$ & $+0.76$\\
MoCo v3-1K& 85.50 & $+1.99$ & $+2.12$\\
iBOT-1K   & 86.46 & $+2.45$ & $+2.06$\\
iBOT-21K  & 88.52 & $+2.06$ & $+1.80$\\
DINO-1K   & 85.36 & $+2.42$ & $+2.03$\\
\multicolumn{4}{l}{\emph{Split-ImageNet-A}}\\
Sup-21K   & 45.85 & $+1.66$ & $+1.57$\\
MoCo v3-1K& 31.80 & $+3.04$ & $+2.41$\\
iBOT-1K   & 37.08 & $+2.97$ & $+3.58$\\
iBOT-21K  & 41.48 & $+4.57$ & $+2.63$\\
DINO-1K   & 36.44 & $+1.86$ & $+0.97$\\
\multicolumn{4}{l}{\emph{5-Datasets}}\\
Sup-21K   & 91.37 & $+2.98$ & $+2.40$\\
MoCo v3-1K& 92.63 & $+1.28$ & $+1.41$\\
iBOT-1K   & 93.80 & $+0.97$ & $+0.85$\\
iBOT-21K  & 93.83 & $+1.13$ & $+0.59$\\
DINO-1K   & 92.52 & $+1.42$ & $+0.75$\\
\hline
\end{tabular}
\end{table}

Table~\ref{tab:grid} reports all of them. \emph{Every cell} improves on both
accuracy axes --- Acc@task by $+0.97$ to $+4.57$, Acc@1 by $+0.59$ to $+4.50$
--- while the baselines span more than a factor of two, from $40.2$ to $93.8$.
The largest gain falls on the weakest backbone, MAE, not the strongest, which is
the direction a method that supplies a second opinion should show. Forgetting,
...
strict superset of the one above. The lower block is the difference from the row
above, i.e.\ the isolated contribution of the component just added.}
\label{tab:cumul}
\begin{tabular}{lrrrrrr}
\hline
& Sup-21K & MoCo-1K & iBOT-1K & iBOT-21K & DINO-1K & MAE-1K\\
\hline
...
\hline
\multicolumn{7}{l}{\emph{Isolated contribution}}\\
...
\subsubsection{The classification stage is the larger component.}
On five of six backbones it contributes $+1.81$ to $+5.76$ points against
$+0.29$ to $+0.74$ for the routing stage. Sup-21K is the sole exception --- and
it is the backbone every design decision in Sect.~\ref{sec:results} was measured
on, which is one more reason not to generalise from a single configuration.
...
and DINO, and removes $0.26$ to $0.54$ points on MoCo and iBOT-1K and $3.71$ on
MAE. That comparison is not fair to the ungated arm, however: $\beta=0.5$ was
...
\emph{removing the gate is better}. Seed 42; MAE was run on ImageNet-R only.}
\label{tab:gate-cross}
\begin{tabular}{lrrrrrrr}
\hline
& Sup-21K & MoCo-1K & iBOT-1K & iBOT-21K & DINO-1K & MAE-1K & Mean\\
\hline
