...
\subsubsection{Second stage: fusion at the classification level.}
...
class scores $s$, and mix the two with a single constant weight
\begin{equation}
\beta = 0.3 .
\label{eq:beta}
\end{equation}

A constant weight is not the obvious choice, and we did not begin with it. Fusion
can change a prediction only by unseating the top-ranked class, so spending the
same share of the decision on a sample the primary head has already named with
certainty as on one it is hesitating over appears wasteful, and we first made
$\beta$ depend on each sample through the margin between the two highest-scoring
classes. Section~\ref{sec:ablation} reports what that cost. Judged at the value
of $\beta$ each arm prefers, the constant weight is the more accurate of the two
on five of six backbones and never has the higher loss; the gated variant's
better forgetting comes from being less accurate at nine of the ten stages rather
than from retaining more. The signal a gate would need does exist -- a linear
combination of margin and rank features separates the samples only the
random-projection head gets right at an AUC of $0.78$ to $0.83$ -- but its
operating point does not survive the move from one task to the next, or from one
backbone to another, and the gain it realises falls from $+1.23$ to $+0.11$ once
it must transfer. We therefore report the simpler rule.
