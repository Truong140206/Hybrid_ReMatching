...
5-Datasets, and no CUB-200), a stray \texttt{.DS\_Store} in
\texttt{training\_scripts/} listing twenty script names, i.e.\ four datasets
times five backbones, and no MAE entry anywhere, together with an MAE loader
that raises \texttt{TypeError} at model construction --- code broken in that way
cannot have been executed. We rebuilt all twenty cells, retraining the entire
HRM-PET pipeline from scratch where checkpoints were missing, and added MAE on
ImageNet-R as a twenty-first cell: it is the configuration where the pre-trained
features are weakest, so it is where a second source of evidence should matter
most, and no published number exists for it.

\begin{table}
\caption{Twenty-one cells: four datasets, the five pre-training configurations
of the original work, plus MAE on ImageNet-R. Acc@1 at seed 42; HRM-PET is
$w=1$, $\beta=0$ and Proposed is $w=0.7$, $\beta=0.5$. 5-Datasets is a sequence
of five datasets and therefore has five tasks by definition; the other three
split their classes into ten. The HRM-PET rows show that the cells span more
than a factor of two.}
\label{tab:grid}
\begin{tabular}{llrrrrrr}
\hline
Dataset & & Sup-21K & MoCo-1K & iBOT-1K & iBOT-21K & DINO-1K & MAE-1K\\
\hline
ImageNet-R & HRM-PET   & 74.02 & 68.47 & 71.89 & 73.29 & 70.23 & 40.20\\
           & Proposed  & 75.51 & 70.37 & 74.41 & 75.84 & 72.91 & 44.69\\
           & $\Delta$  & $+1.49$ & $+1.89$ & $+2.52$ & $+2.55$ & $+2.68$ & $+4.50$\\
           & Rel.\ ER  & 5.7\% & 6.0\% & 9.0\% & 9.5\% & 9.0\% & 7.5\%\\
\hline
...
the direction a method that supplies a second opinion should show.
...
Baseline $+$ routing ($w=0.7$)          & 74.31 & 68.81 & 72.46 & 73.91 & 70.97 & 42.64\\
Baseline $+$ class ($\beta=0.5$)        & 74.53 & 70.70 & 74.96 & 75.72 & 72.91 & 48.39\\
Baseline $+$ routing $+$ class          & 74.55 & 70.63 & 74.95 & 75.72 & 72.87 & 48.40\\
Baseline $+$ routing $+$ class $+$ gate & 75.51 & 70.37 & 74.41 & 75.84 & 72.91 & 44.69\\
\hline
\multicolumn{7}{l}{\emph{Isolated contribution over the baseline}}\\
routing alone & $+0.29$ & $+0.34$ & $+0.57$ & $+0.62$ & $+0.74$ & $+2.44$\\
class alone   & $+0.51$ & $+2.23$ & $+3.07$ & $+2.43$ & $+2.68$ & $+8.19$\\
\multicolumn{7}{l}{\emph{Marginal contribution given the rest}}\\
routing given class & $+0.02$ & $-0.07$ & $-0.01$ & $\ \ 0.00$ & $-0.04$ & $+0.01$\\
gate given both     & $+0.96$ & $-0.26$ & $-0.54$ & $+0.12$ & $+0.04$ & $-3.71$\\
\hline
\end{tabular}
\end{table}
...
isolated contribution is largest where the backbone is weakest: $+8.19$ on MAE
against $+0.51$ on Sup-21K, the backbone every design decision in
