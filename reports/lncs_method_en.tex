...
% VERIFY BEFORE SUBMISSION. The nine entries below were added with the Related
% Work section. Venue and year are as cited in the sources we read, but the
% author lists of ref_coda, ref_adam, ref_icarl, ref_bic, ref_wa and ref_lucir
% were written from memory and each needs one look at the paper. ref_dlepem is
% missing its authors entirely: the publisher page refused automated access, so
% the field is left blank on purpose rather than filled with a guess.
