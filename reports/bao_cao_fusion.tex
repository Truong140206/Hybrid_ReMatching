...
được: hai bộ dữ liệu đầu chênh $2$ đến $4$ điểm, riêng ImageNet-R chênh thêm
khoảng $12$ điểm nữa. Khác biệt đó nay đã được giải thích, và nguyên nhân nằm ở
phép so sánh chứ không ở cài đặt.

Chúng tôi lần lượt loại trừ từng thành phần bằng thí nghiệm trực tiếp. Dữ liệu
không thiếu ($200$ lớp, $24\,000$ ảnh huấn luyện và $6\,000$ ảnh kiểm tra). Đặc
trưng không bị thích nghi ngầm, kiểm chứng bằng một bộ trích tạo mới. Không phải
hệ quả của trình tự học liên tục, do tính bất biến thứ tự của $G$ và $C$. Không
phải do tăng cường dữ liệu, vì thống kê được tích lũy trên biến đổi tất định
đúng như RanPAC. Không phải sai khoảng giá trị ảnh đầu vào: đo cả hai phía trên
ImageNet-R với bộ trích trần cho $55.79$ khi để nguyên và $56.36$ khi sửa, tức
bản sửa đáng $+0.56$ điểm --- có thật nhưng nhỏ, và $56.36$ ở bảng vốn đã là con
số đã sửa.

\textbf{Bước quyết định là bỏ hẳn phép chiếu.} Đặt số chiều chiếu bằng không thì
phương pháp thu về đúng một phép hồi quy ridge trên đặc trưng $768$ chiều đóng
băng, tức một bộ phân lớp tuyến tính, và RanPAC công bố số cho đúng cấu hình đó.
Kết quả: CUB-200 đạt $86.42$ so với $86.8$ của họ, trong khi ImageNet-R chỉ đạt
$54.02$ so với $67.6$. Cùng một đoạn mã, cùng bộ trích, cùng cách chọn $\lambda$
--- một bộ khớp, một bộ lệch $13.6$. Khoảng cách sống sót cả khi đã bỏ toàn bộ
phần chiếu ngẫu nhiên, nên đầu phân lớp không phải nguyên nhân. Dữ liệu cũng
không, vì baseline HRM-PET của chúng tôi trên ImageNet-R đạt $73.94$ so với
$73.86$ mà bài gốc công bố.

Chỉ còn lại câu hỏi checkpoint tiền huấn luyện. ImageNet-R gồm đúng $200$ lớp
\emph{của ImageNet-1k} được vẽ lại dưới dạng tranh, tượng và hoạt hình, nên một
mạng xương sống đã tinh chỉnh trên ImageNet-1k có sẵn đặc trưng khớp với đúng
$200$ nhãn ấy; CUB-200 là ảnh chim, không thuộc tập lớp $1$k, nên không hưởng
lợi tương ứng. Chúng tôi kiểm bằng cách trỏ \emph{riêng} ảnh chụp đóng băng sang
checkpoint ImageNet-21K đã tinh chỉnh trên ImageNet-1k, giữ nguyên mọi thành
phần khác. Bảng \ref{tab:backbone} ghi kết quả.

\begin{table}[htbp]
\centering
\caption{Cùng cấu hình bỏ chiếu, chỉ đổi checkpoint của bộ trích đóng băng.
Acc@1, seed 42.}
\label{tab:backbone}
\begin{tabular}{@{}lrrr@{}}
\toprule
Bộ dữ liệu & ImageNet-21K & 21K + 1k & Chênh \\
\midrule
...
Hai điều đọc ra. Thứ nhất, mức tăng của ImageNet-R lớn gấp mười ba lần mức tăng
của CUB-200, đúng như dự đoán từ việc lớp của nó trùng với tập lớp $1$k, nên đây
không phải hiệu ứng "checkpoint tốt hơn nói chung". Thứ hai, con số thu được là
$68.03$ so với $67.6$ mà RanPAC công bố --- lệch $0.43$. Khoảng cách $13.6$ điểm
biến mất.

Kết luận: bảng đối chiếu trước đây so kết quả trên mạng xương sống ImageNet-21K
thuần với con số nhiều khả năng được đo trên checkpoint đã tinh chỉnh ImageNet-1k.
Đó là sai lệch của \emph{phép so sánh}, không phải của cài đặt. Chúng tôi giữ
mạng xương sống $21$K cho toàn bộ báo cáo, vì đó là mạng mà HRM-PET được huấn
luyện cùng và mọi số ở Mục \ref{sec:results} đều dựa trên nó.

Bản gốc xác nhận điều này là có cơ sở, và nói khá rõ. Phần thí nghiệm cho biết họ
dùng \emph{hai} mô hình ViT-B/16: một tiền huấn luyện trên ImageNet-21K, một tinh
chỉnh có giám sát trên ImageNet-1K. Phụ lục của bài viết thẳng rằng ``cùng một
mạng xương sống không phải là tốt nhất cho mọi bộ dữ liệu'', rồi nêu ví dụ: bản
tinh chỉnh ImageNet-1K tốt nhất cho ImageNet-A, còn bản ImageNet-21K tốt nhất cho
CIFAR100 và OmniBenchmark. ImageNet-R không được nêu tên trong câu đó.

Hai hệ quả. Thứ nhất, bảng kết quả chính báo cáo lựa chọn tốt nhất \emph{theo
từng bộ dữ liệu}, nên hai cột khác nhau của cùng một hàng có thể đến từ hai
checkpoint khác nhau. Thứ hai, phần đối chiếu giữa hai mạng xương sống chỉ được
trình bày dưới dạng \emph{đồ thị}, không phải bảng số, nên không có con số theo
từng checkpoint để đọc ra.

Vì vậy chính phép đo ở Bảng \ref{tab:backbone} mới là thứ cung cấp con số còn
thiếu. Nó cũng nhất quán với những gì bài nói: trên CUB-200, nơi kết quả $21$K
thuần của chúng tôi chỉ kém $0.4$ điểm, hai bên gần như chắc chắn dùng chung một
checkpoint; còn trên ImageNet-R, bản $21$K thuần kém $13.6$ điểm trong khi bản
tinh chỉnh ImageNet-1K vượt $0.43$ điểm. Đó đúng là điều phải xảy ra nếu con số
ImageNet-R công bố được đo trên bản tinh chỉnh --- và ImageNet-R gồm đúng $200$
lớp của ImageNet-1k thì đó cũng là lựa chọn hợp lý về phía họ.
