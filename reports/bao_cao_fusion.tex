...
\subsubsection{Vì sao phép affine sử dụng mô-men của $\ell$}

Phép biến đổi \eqref{eq:affine} là biến đổi affine theo từng mẫu với hệ số nhân
dương, do đó đơn điệu tăng: thứ tự xếp hạng giữa các lớp được bảo toàn và mọi chỉ
số độ chính xác không thay đổi. Vai trò duy nhất của nó là đưa thang đo của logit
về mức so sánh được với baseline khi tính cross-entropy.

Việc chọn mô-men của $\ell$ làm mốc không tùy tiện. Thay $\beta_i = 0$ vào
\eqref{eq:affine} ta được
\begin{equation}
\hat{\ell} = z(\ell) \cdot \mathrm{std}(\ell) + \mathrm{mean}(\ell) = \ell ,
\end{equation}
tức phương pháp trả về đúng logit gốc. Chỉ với mốc này thì $\beta$ mới là một
tham số liên tục nội suy về baseline, tương tự vai trò của $w$ ở tầng thứ nhất
(Mục \ref{sec:sanity}). Ngoài ra, giữa hai nguồn chỉ có $\ell$ mang thang đo có ý
nghĩa xác suất, vì nó là đầu ra của một bộ phân lớp huấn luyện bằng cross-entropy,
trong khi $s$ là giá trị hồi quy chưa chuẩn hóa.

Một hệ quả của thiết kế này: phép chuẩn hóa $z(\cdot)$ bất biến với mọi biến đổi
affine dương của đầu vào, nên $z(as + b) = z(s)$ với mọi $a > 0$. Mọi hệ số nhiệt
độ vô hướng áp lên $s$ đều bị triệt tiêu, và đó là lý do chúng tôi không áp dụng
hiệu chỉnh nhiệt độ cho nguồn thứ hai.
...
\subsection{Phân tách đóng góp của hai tầng}
\label{sec:two-channels}

Nguồn tín hiệu thứ hai tác động lên kết quả cuối cùng qua hai đường khác nhau.
Tầng thứ nhất tác động \emph{gián tiếp}: nó thay đổi bộ tham số LoRA được nạp,
do đó thay đổi đặc trưng đưa vào đầu phân lớp. Tầng thứ hai tác động
\emph{trực tiếp}: điểm số $s$ được trộn thẳng vào logit. Bảng \ref{tab:channels}
tách riêng hai đóng góp này.

\begin{table}[htbp]
\centering
\caption{Đóng góp của từng tầng, đo bằng Acc@1 trên ImageNet-R, seed 42. Cấu hình
ở hàng giữa tương ứng với $\beta = 0$, tại đó $\hat{\ell} = \ell$ và tầng thứ hai
bị vô hiệu hóa hoàn toàn.}
\label{tab:channels}
\begin{tabular}{@{}lrl@{}}
\toprule
Cấu hình & Acc@1 & Đường tác động \\
\midrule
HRM-PET & 74.02 & --- \\
Chỉ hợp nhất định tuyến & 74.31 & gián tiếp, $+0.29$ \\
Bổ sung hợp nhất phân lớp & \textbf{75.42} & cả hai, $+1.40$ \\
\bottomrule
\end{tabular}
\end{table}

Đường gián tiếp chỉ đóng góp $+0.29$ điểm, trong khi đường trực tiếp bổ sung thêm
$+1.11$ điểm. Đây là dạng định lượng của hạn chế thứ hai đã phân tích ở Mục
\ref{sec:analysis}: do đầu phân lớp dùng chung trải trên toàn bộ các lớp đã quan
sát, một phần lớn thông tin đưa vào qua đường định tuyến bị hấp thụ mà không
chuyển hóa thành độ chính xác. Kết quả này là cơ sở thực nghiệm cho việc bổ sung
tầng thứ hai thay vì chỉ tinh chỉnh tầng thứ nhất.
