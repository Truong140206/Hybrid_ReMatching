...
and we claim no improvement there. The gain does not shrink as the backbone
weakens: across six ViT-B/16 backbones differing in pretraining the
baseline spans $27.8$ points while the gain stays between $+2.18$ and
$+3.95$, and is largest on the weakest backbone. The added compute is exactly one forward pass
per sample.
\end{abstract}
...
--- both TII and the LoRA pool, ten tasks, from scratch --- on five further
backbones, then measured baseline and proposed method through one identical
code path at one identical seed. Table~\ref{tab:backbones} records the result.

\begin{table}[htbp]
\centering
\caption{Six ViT-B/16 backbones on Split-ImageNet-R, seed 42, ten tasks.
Baseline is HRM-PET ($w = 1$, $\beta = 0$); Proposed is $w = 0.7$,
$\beta = 0.5$. Each backbone was retrained from scratch. Rows are ordered by
baseline Acc@task; MAE is set apart because it falls outside the band the other
five occupy.}
\label{tab:backbones}
...
\textbf{The gain does not shrink as the backbone weakens --- it grows.} Five of
the backbones sit in a narrow band: their baselines span $5.60$ points of
Acc@task, from $73.82$ to $79.42$, while the gain spans $0.87$ points, from
$+2.18$ to $+3.05$. Inside that band what is added varies about one sixth as
much as the thing it is added to, and follows no monotone trend: the backbone
with the largest gain (iBOT-$1$K) and the one with the smallest (Sup-$21$K) have
baselines $0.21$ points apart. MAE lies outside the band, $22.22$ points below
its nearest neighbour, and yields the largest gain in the table: $+3.95$ points
of Acc@task and $+4.50$ of Acc@1. Cutting the baseline nearly in half therefore
does not shrink what the second evidence source contributes; it enlarges it.
That matches the mechanism analysed in Sect.~\ref{sec:analysis}: the
routing-level fusion helps most exactly where the original router is most often
wrong.

\textbf{All six improve on both accuracy axes.} There is no counterexample among
the twelve cells. Within the narrow band the smallest gain falls on Sup-$21$K,
the only supervised-pretrained backbone. With one seed per backbone we record
the observation and do not interpret its cause.
...
The four backbones measured afterwards all point the other way: DINO $3.44$ to
$2.87$, iBOT-$1$K $4.44$ to $3.53$, iBOT-$21$K $4.24$ to $3.82$, MAE $10.03$ to
$9.53$, alongside Sup-$21$K $3.28$ to $3.05$. Five of six improve, and MoCo v3
is the exception rather than the rule. This does not promote retention into a
claim: the \emph{no change distinguishable from zero} conclusion of
Sect.~\ref{sec:results} stands, because it rests on four seeds whereas each
backbone here has one.

\textbf{On the MAE row specifically.} Two things must be said plainly. First, a
baseline of $51.60$ is a model that is broken for practical purposes --- its
Loss is $11.15$ against $1.23$ for Sup-$21$K --- so the row should be read as a
stress test, not as a configuration anyone would deploy. Second, that figure
depends on a choice of ours. The original code passes MAE two construction
arguments this codebase's ViT class does not accept, one of which indicates
pooling the final sequence by averaging rather than by the cls token --- which
is how MAE is normally used, since MAE pretraining gives the cls token no
supervision at all. We kept token pooling, as for the other five. Average
pooling would almost certainly raise MAE's baseline, but MAE would then be the
one row travelling a different code path, and precisely the property that gives
this table its value would be lost.

\textbf{The Sup-$21$K row is a single seed}, not the four-seed mean of
Table~\ref{tab:main}, so that all six rows share one aggregation. Its two
...
\textbf{Scope.} One benchmark, one seed per backbone, six backbones. And that
benchmark is the one all three hyperparameters were chosen on
(Sect.~\ref{sec:limits}), so this table tests robustness to the backbone and
adds nothing to the evidence that is independent of the tuning --- that remains
the $11$ cells of CIFAR-100 and CUB-200. Cost limits the number of rows: each
one requires retraining from scratch, roughly $1000$ seconds for TII and $6100$
for the LoRA pool on one RTX 4090 GPU, close to two hours per backbone.
...
again on six ViT-B/16 backbones differing in pretraining, all six improve
on both accuracy axes, and the largest gain falls on the backbone with the
lowest baseline --- $22$ points below the next one --- not on the
strongest.
