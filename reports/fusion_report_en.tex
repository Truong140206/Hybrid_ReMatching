...
Table~\ref{tab:nophase1} records a discrepancy we cannot yet explain. The first
two benchmarks differ by $2$ to $4$ points, which is attributable to our fixing
$\lambda$ instead of cross-validating it per session as RanPAC does, together
with small differences in image preprocessing. ImageNet-R alone differs by a
further $12$ points or so.

We ruled out five causes by direct experiment: the data is not missing ($200$
classes, $24\,000$ training and $6\,000$ test images); the features are not
implicitly adapted (verified with a freshly constructed extractor, matching digit
for digit); it is not a consequence of the continual-learning sequence (by the
order invariance of $G$ and $C$); it is not $\lambda$ (swept over
$\{10^2,\dots,10^6\}$, with an interior maximum of $58.10$); and it is not data
augmentation perturbing the statistics (the statistics are accumulated over
deterministic transforms, as in RanPAC).

\textbf{One candidate remains, and we have not measured it on ImageNet-R.} The
codebase's transform builder calls \texttt{ToTensor} without \texttt{Normalize},
so images arrive in $[0,1]$ while the ImageNet-21K ViT-B/16 checkpoint was
trained on $[-1,1]$. HRM-PET is immune because it trains on whatever features it
is given; a training-free head is not. We do have an option that corrects the
range, but it is \emph{off by default}, so every number in
Table~\ref{tab:nophase1} was measured with the fault present. On the one dataset
where it was measured, correcting the range is worth $+2.7$ points on CUB-200
frozen features --- larger than CUB-200's own $2$-point gap to RanPAC.
ImageNet-R is the most exposed by argument, since stylised renditions already sit
far from the pre-training distribution so a range error compounds rather than
being absorbed; but that is an argument, and the corresponding measurement has
not been run. We therefore do \emph{not} claim this cause is ruled out.
...
tasks. Our validation error follows that formula exactly, but \textbf{the data it
ranges over does not}: our $80{:}20$ statistics accumulate across \emph{every}
task seen so far, whereas the original rebuilds them from scratch on the current
task alone. This is a genuine divergence, and it biases in exactly the direction
observed: the norm of $G$ grows linearly with sample count and the optimal ridge
grows with the norm of $G$, so sweeping over ten pooled tasks favours a larger
$\lambda$ than sweeping over one. The selected value sits near the top of the
swept range, which is consistent with that bias. What follows is therefore the
result of a \emph{variant} of the procedure, not of the original. The validation
error expands to
...
$\lambda$, seventeen times the cost of one complete run.

That explanation is, however, no longer the only one available, and we cannot
yet separate the two. Both criteria were evaluated on the same pooled statistics,
so the scope divergence noted above biases both of them identically --- their
agreement therefore says nothing about whether the pooling is what pushed the
choice upward. Rebuilding the held-out statistics per task, which is the
original's actual procedure, is a small change and has not been run. Until it
is, the honest statement is that our variant selects $10^5$ and costs $0.43$
points, not that automatic selection as such fails here.
