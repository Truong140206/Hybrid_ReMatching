...
and we claim no improvement there. Across the original's full grid of four
benchmarks by five backbones, plus MAE as a twenty-first cell, all twenty-one
cells improve on both accuracy axes, over baselines spanning $40.2$ to $93.8$
points of Acc@1, and the largest gain falls on the weakest backbone. The added compute is exactly one forward pass
per sample.
\end{abstract}
...
\section{The Full Grid: Four Benchmarks and Five Backbones}
\label{sec:grid}

Every number in Sect.~\ref{sec:results} rests on one backbone and three
benchmarks. If the improvement existed only by virtue of that pretraining, or
of those benchmarks, the conclusion would be far narrower than it appears.

\textbf{What the original covers.} Its abstract reports experiments on
\emph{four datasets under five pre-trained settings} without saying which. Three
traces in the released code answer that. First, the authors' code drop contains
exactly four pairs of configuration files: CIFAR-100, ImageNet-R, ImageNet-A
and 5-Datasets --- not CUB-200, which is our own addition. Second, a
\texttt{.DS\_Store} left behind in \texttt{training\_scripts/} still records the
names of every script that directory once held: twenty files, exactly four
benchmarks times five backbones, and those five are Sup-$21$K, MoCo v3-$1$K,
iBOT-$1$K, iBOT-$21$K and DINO-$1$K. Third, none of them mentions MAE, and the
MAE loader in the released code raises \texttt{TypeError} on its model
construction line --- code broken that way is evidence it was never executed.

We assembled all twenty cells, retraining the entire HRM-PET pipeline --- both
TII and the LoRA pool, from scratch --- for each one that was missing, then
measuring baseline and proposed method through one identical code path at one
identical seed. MAE on ImageNet-R is added as a twenty-first cell, because it
is the mechanically hardest case and we wanted to see where it broke.
Table~\ref{tab:grid} is the result, read out of the evaluation logs rather than
transcribed.

\begin{table}[htbp]
\centering
\small
\caption{Twenty-one cells: four benchmarks, the original's five backbones, plus
MAE on ImageNet-R. Seed 42; Baseline is HRM-PET ($w = 1$, $\beta = 0$),
Proposed is $w = 0.7$, $\beta = 0.5$. 5-Datasets has five tasks, the other three
have ten.}
\label{tab:grid}
...
\textbf{Twenty-one of twenty-one cells improve, on both accuracy axes.} There is
no counterexample among the forty-two comparisons. What matters is not the size
of the gain --- it is not large --- but the range of the baseline it sits on:
$40.20$ to $93.83$ points of Acc@1, more than a factor of two, across four
benchmarks and six kinds of pretraining.

\textbf{The gain shrinks where there is no room left.} Mean Acc@1 gain per
benchmark: ImageNet-A $+2.23$, ImageNet-R $+2.23$, CIFAR-100 $+1.75$,
5-Datasets $+1.20$. 5-Datasets has the highest baselines ($91.4$--$93.8$) and
the smallest gains. That is the correct behaviour for a method that repairs
routing errors: the fewer samples are routed wrongly, the less there is to
repair.

\textbf{Supervised pretraining gains less, but not everywhere.} On ImageNet-R,
CIFAR-100 and ImageNet-A, Sup-$21$K shows the smallest Acc@task gain of the five
backbones in its row. On 5-Datasets it shows the largest ($+2.98$ against
$+0.97$ to $+1.42$). Three benchmarks out of four pointing one way is not enough
to state as a rule, so we record the observation together with its exception.

\textbf{The retention axis: $16/21$ improve.} The five that worsen are
ImageNet-R MoCo v3 ($3.10 \to 3.69$), ImageNet-A MoCo v3 ($3.99 \to 4.04$),
ImageNet-A iBOT-$1$K ($4.73 \to 5.12$), ImageNet-A DINO ($3.32 \to 5.26$) and
5-Datasets iBOT-$21$K ($0.40 \to 0.57$). Three of the five are on ImageNet-A,
the benchmark whose sixth task holds $68$ evaluation images across $20$ classes.
We still claim no improvement on this axis: the \emph{no change distinguishable
from zero} conclusion of Sect.~\ref{sec:results} rests on four seeds, and this
grid has one.

\textbf{The MAE case, and a prediction of ours that was wrong.} Before running
it we argued that MAE was the one case with a mechanical reason to fail: MAE
pretraining gives the cls token no supervision, its frozen features are
notoriously weak under linear probing, and our second evidence source is built
on exactly those frozen features. The measurement refutes that. MAE gives the
lowest baseline of the twenty-one cells ($40.20$ Acc@1) and \emph{the largest
Acc@1 gain} ($+4.50$). The reading consistent with the rest of the table is that
the weaker the frozen features, the more a second-order random projection can
still extract from them --- not less. We leave the wrong prediction in the
report because it is the informative part: the mechanism we expected to limit
the method is the one that makes it work best.

\textbf{Scope, and four ways this grid differs from the original.} First, the
whole grid is measured at a \emph{single seed} (42); the four-seed table in
Sect.~\ref{sec:results} still covers only three benchmarks on Sup-$21$K.
Second, the ImageNet-A $80/20$ split is ours, not theirs: the released code
splits at random over the flat sample list without stratifying, which leaves
eight classes with no test images at all and makes the dataset impossible to
construct; we split within each class and fix the seed. Their split was unseeded
and so cannot be reproduced even in principle, which means \emph{our ImageNet-A
numbers are not directly comparable to the original's}. Third, we use LoRA rank
$8$ where their released script uses rank $5$; changing it would invalidate
every number already measured, and it does not affect the claim, since every
comparison is baseline against proposed at the same rank. Fourth, three
ImageNet-A cells (iBOT-$1$K, iBOT-$21$K, DINO) were trained after we added a
guard for singular class covariances, without which they died at the third
task; the guard fires only where the previous code raised, so the other
eighteen cells went through an unchanged code path.

\section{Ablation Studies}

\subsection{Cumulative Ablation, in the Original's Shape}
\label{sec:cumulative}

The original ablates its components \emph{cumulatively}: each row adds exactly
one component to the row above, measured on ImageNet-R, with its five backbones
as columns. Our own ablations, in the subsections that follow, cover as much
ground but are spread across separate tables and only on Sup-$21$K, which makes
comparing against theirs harder than it needs to be. Table~\ref{tab:cumul}
rebuilds that shape for our two stages.

\begin{table}[htbp]
\centering
\small
\caption{Cumulative ablation on Split-ImageNet-R, seed 42, Acc@1. Each row is a
strict superset of the one above. The lower half is the difference from the
preceding row, that is, what the component just added is worth.}
\label{tab:cumul}
\begin{tabular}{@{}lrrrrrr@{}}
\toprule
& Sup-$21$K & MoCo-$1$K & iBOT-$1$K & iBOT-$21$K & DINO-$1$K & MAE-$1$K \\
\midrule
Baseline ($w = 1$, $\beta = 0$)      & 74.02 & 68.47 & 71.89 & 73.29 & 70.23 & 40.20 \\
$+$ routing stage ($w = 0.7$)        & 74.31 & 68.81 & 72.46 & 73.91 & 70.97 & 42.64 \\
$+$ class stage ($\beta = 0.5$)      & 74.55 & 70.63 & 74.95 & 75.72 & 72.87 & 48.40 \\
$+$ margin gate (full)               & 75.51 & 70.37 & 74.41 & 75.84 & 72.91 & 44.69 \\
\midrule
\multicolumn{7}{@{}l}{\emph{What each component is worth}}\\
$+$ routing stage & $+0.29$ & $+0.34$ & $+0.57$ & $+0.62$ & $+0.74$ & $+2.44$ \\
$+$ class stage   & $+0.24$ & $+1.82$ & $+2.49$ & $+1.81$ & $+1.90$ & $+5.76$ \\
$+$ margin gate   & $+0.96$ & $-0.26$ & $-0.54$ & $+0.12$ & $+0.04$ & $-3.71$ \\
\bottomrule
\end{tabular}
\end{table}

\textbf{The class stage, not the routing stage, is the main component.} On five
of the six backbones the class stage is worth $+1.81$ to $+5.76$ points while
the routing stage is worth $+0.29$ to $+0.74$. Sup-$21$K is the sole exception
--- and it is the backbone every design decision in Sect.~\ref{sec:results} was
measured on, which is one more reason not to generalise from a single one.

\textbf{The margin gate helps on the backbone it was tuned on and hurts
elsewhere.} At the published $\beta$ it is worth $+0.96$ on Sup-$21$K, is
neutral on iBOT-$21$K and DINO, costs $0.26$ to $0.54$ points on MoCo and
iBOT-$1$K, and costs $3.71$ on MAE.

\textbf{That comparison is unfair to the ungated arm, and we checked.} The value
$\beta = 0.5$ was chosen \emph{with the gate in place}; forcing the ungated head
to reuse it compares a tuned configuration against an untuned one. We swept
$\beta \in \{0.3, 0.5, 0.8, 1.0\}$ for both arms on all six backbones. The
result separates into two distinct findings.

First, \emph{the gate makes the method nearly insensitive to $\beta$}. Across
the sweep, gated Acc@1 varies by at most $1.17$ points and at least $0.13$; the
ungated arm varies by at least $3.89$ and at most $5.62$. That is exactly what
the gate was designed to do.

Second, \emph{at each arm's own best $\beta$ the ungated arm wins on five of six
backbones} on ImageNet-R --- and the sixth, Sup-$21$K, loses by $0.09$, which is
noise. The ungated optimum is $\beta = 0.3$ on five of the six.

\begin{table}[htbp]
\centering
\small
\caption{Acc@1 difference from dropping the gate and lowering $\beta$ to $0.3$,
against the published configuration (gate on, $\beta = 0.5$). Positive means
\emph{dropping the gate is better}. Seed 42.}
\label{tab:gate-cross}
\begin{tabular}{@{}lrrrrrrr@{}}
\toprule
& Sup-$21$K & MoCo-$1$K & iBOT-$1$K & iBOT-$21$K & DINO-$1$K & MAE-$1$K & Mean \\
\midrule
...
\textbf{Across the full grid the picture reverses by benchmark.}
Table~\ref{tab:gate-cross} extends the comparison to all four. Dropping the gate
wins on ImageNet-R and CIFAR-100, roughly ties on ImageNet-A, and \emph{loses
five out of five} on 5-Datasets. Averaged over the grid's twenty cells, dropping
it is worth $+0.25$ points.

That shape matches what the gate was built to do. It mixes the class stage only
into samples the original model is \emph{not} already confident about. On
5-Datasets the baseline sits at $91.4$--$93.8$ Acc@1, so nearly every sample is
already right and already confident --- precisely the samples the gate protects.
On ImageNet-R ($68$--$74$) and CIFAR-100 ($85$--$90$) there are many wrong ones
left to fix, and mixing unconditionally pays off.

\textbf{We keep the gate, and say why.} Three reasons. It wins on 5-Datasets,
the benchmark furthest from where it was tuned, which makes that transfer
evidence rather than fitting evidence. It makes the method insensitive to
$\beta$, which matters because we do not tune $\beta$ per benchmark. And what it
costs is half a point on two benchmarks against up to a full point gained on
another.

\textbf{We came close to removing it on ImageNet-R evidence alone.} We record
that because it is the exact trap Sect.~\ref{sec:limits} warns about, and it
nearly caught us immediately after we wrote the warning. The claim in an earlier
version of this report --- that the gate \emph{eliminates the only systematic
regression we observed} --- was wrong: it was drawn from one backbone, one
benchmark, at a $\beta$ the ungated arm had never been tuned for.

Finally, the two tables here and Table~\ref{tab:beta} below measure different
things and are both correct: dropping the gate gives higher Acc@1 on ImageNet-R
and CIFAR-100, while CIFAR-100 Loss worsens by $+0.019 \pm 0.003$ over three
seeds. Mixing unconditionally fixes more labels and degrades the probability
...
What persuades us the result is not a property of one backbone or one
benchmark: across the original's full four-by-five grid, plus MAE as a
twenty-first cell, no cell moves the wrong way on either accuracy axis,
over baselines spanning more than a factor of two --- $40.2$ to $93.8$
points of Acc@1. And the largest gain falls on the backbone with the lowest
baseline, not on the strongest.

The margin gate mixes the second stage only into samples the original model is
not already confident about, and adds no hyperparameter. The cumulative ablation
in Sect.~\ref{sec:cumulative} shows its value depends on the benchmark:
dropping it and lowering $\beta$ to $0.3$ is worth about half a point on
ImageNet-R and CIFAR-100, and loses on all five 5-Datasets cells, where the
baseline is near saturation. We keep it because it wins on the benchmark
furthest from where it was tuned, and because it makes the method nearly
insensitive to $\beta$ --- varying by at most $1.17$ points against at least
$3.89$ without it.
