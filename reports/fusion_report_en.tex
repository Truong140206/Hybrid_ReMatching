...
The original confirms that this matters. Its experimental section states that it
uses \emph{two} ViT-B/16 models: one pre-trained on ImageNet-21K, and one with
supervised fine-tuning on ImageNet-1K. Neither results-table caption records
which checkpoint produced which number, and neither claims to report the better
of the two. The comparison was therefore never guaranteed to be like-for-like,
and it is the measurement in Table~\ref{tab:backbone} that identifies the
checkpoint: the ImageNet-1k fine-tuned model reproduces the published figure to
within $0.43$ points, while the 21K-only model falls $13.6$ short.
