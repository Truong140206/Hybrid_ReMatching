...
We ruled out six causes by direct experiment: the data is not missing ($200$
...
$\{10^2,\dots,10^6\}$, with an interior maximum of $58.10$); it is not data
augmentation perturbing the statistics (the statistics are accumulated over
deterministic transforms, as in RanPAC); and it is not a wrong input value range.

The last of these is worth spelling out, because it is a genuine fault in the
codebase. The transform builder calls \texttt{ToTensor} without
\texttt{Normalize}, so images arrive in $[0,1]$ while the ImageNet-21K ViT-B/16
checkpoint was trained on $[-1,1]$. HRM-PET is immune because it trains on
whatever features it is given; a training-free head is not. We measured both
sides directly on ImageNet-R with the bare extractor: leaving the range wrong
gives Acc@1 $55.79$, correcting it gives $56.36$. The correction is therefore
worth $+0.56$ points --- real, but one twenty-seventh of the gap to be
explained, and the $56.36$ in Table~\ref{tab:nophase1} is already the
\emph{corrected} figure. Note also that this is far below the $+2.7$ points the
same correction buys on CUB-200, so it does not transfer between datasets.
