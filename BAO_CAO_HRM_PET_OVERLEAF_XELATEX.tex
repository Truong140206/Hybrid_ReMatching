...
% xelatex -interaction=nonstopmode BAO_CAO_HRM_PET_OVERLEAF_XELATEX.tex
...
% Bố cục được tối ưu cho XeLaTeX trên Overleaf. Không dùng [shrink]
% toàn cục vì cách đó làm cỡ chữ giữa các slide thiếu nhất quán.
...
\setbeamertemplate{frametitle}{%
  \vspace{0.16cm}\hspace{0.08cm}\insertframetitle\par
  \vspace{0.02cm}\hspace{0.08cm}\color{HRMBlue}\rule{1.05cm}{1.4pt}
}
\setbeamertemplate{footline}{%
  \leavevmode
  \hbox{%
    \begin{beamercolorbox}[wd=.86\paperwidth,ht=2.25ex,dp=0.8ex,leftskip=.4cm]{normal text}
      \color{HRMGray}\tiny Tài liệu tự học · Cải tiến HRM-PET
    \end{beamercolorbox}%
    \begin{beamercolorbox}[wd=.14\paperwidth,ht=2.25ex,dp=0.8ex,rightskip=.4cm plus1fil]{normal text}
      \color{HRMGray}\tiny\insertframenumber/\inserttotalframenumber
    \end{beamercolorbox}%
  }
}
...
\newcommand{\remember}[1]{%
  \par\vspace{0.12cm}%
  \begin{beamercolorbox}[wd=\linewidth,sep=0.11cm,leftskip=0.08cm,rightskip=0.08cm,rounded=true]{block body}
    \footnotesize\textbf{\color{HRMBlue}Ý cần nhớ:}\,\color{HRMDark}#1
  \end{beamercolorbox}
}
...
% 14
\begin{frame}{CFS bước 1: học MLP contrastive theo lớp}
  \small
  Với feature thật $z_i$ của lớp $c$, MLP hai tầng tạo embedding chuẩn hóa
  \[
    u_i=\frac{f_{\mathrm{CFS}}(z_i)}{\|f_{\mathrm{CFS}}(z_i)\|_2}.
  \]
  MLP được học bằng cách giảm mức tương đồng giữa các feature khác nhau:
  \[
  ...
  \begin{columns}[T,onlytextwidth]
    \begin{column}{0.48\textwidth}
      \textbf{Tác dụng}\par
      Các feature trong cùng lớp được trải ra trong không gian embedding, nhờ đó dễ đo độ đa dạng hơn.
    \end{column}
    \begin{column}{0.48\textwidth}
      \textbf{Triển khai}\par
      MLP hai tầng, LeakyReLU, hidden tối đa 512, SGD; dùng tối đa 1024 feature cho mỗi lớp.
    \end{column}
  \end{columns}
  \remember{MLP không phân lớp; nó chỉ tạo thước đo diversity để chọn replay.}
  \sourceNote{utils.py: CFSContrastiveMLP and train\_cfs\_model; paper CFS Eq. 6.}
\end{frame}

% 15
\begin{frame}{CFS bước 2: chọn candidate đa dạng}
  \small
  \centering
  \resizebox{0.92\linewidth}{!}{%
    \begin{tikzpicture}[node distance=0.38cm]
      \node[flow] (a) {Sinh nhiều candidate\\từ Gaussian};
      \node[flow] (b) [right=of a] {Chiếu candidate\\qua MLP CFS};
      \node[flow] (c) [right=of b] {Khởi tạo\\tập đã chọn};
      \node[flownew] (d) [right=of c] {Chọn candidate có\\độ giống TB thấp nhất};
  ...
  \vspace{0.22cm}
  Nếu tập đã chọn là $S$, candidate tiếp theo là
  \[
  ...
  \begin{itemize}\setlength\itemsep{0.08cm}
    \item \texttt{candidate\_multiplier}: số candidate được sinh so với số replay cần giữ.
    \item Bản tốt nhất dùng khởi tạo ngẫu nhiên; mean initialization và distribution filter không cải thiện.
  \end{itemize}
  \remember{CFS chỉ chọn một subset đa dạng hơn trong cùng phân phối lớp; nó không tạo nhãn hay lớp mới.}
\end{frame}
