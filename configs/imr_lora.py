...
def get_args_parser(subparsers):
    ...
    subparsers.add_argument('--rp_route_oracle', action='store_true', help="an audit, not a method: replace the fused route with the true task, so the remaining error can be split into what routing costs and what the classifier costs. The error budget on ImageNet-R Sup-21K is 24.58 points, of which perfect choice between the two heads recovers 3.48 and both heads fail on 21.10 -- and 20.03 percent of samples are routed to the wrong task, which is close enough to 21.10 to be worth measuring rather than assuming")
    subparsers.add_argument('--rp_task_debias', default=0.0, type=float, help="remove this fraction of each task block's own mean from the routed scores, per sample, before class fusion. The shared head is task-recency biased -- on ImageNet-R the mean standardised score of the newest task's classes exceeds the oldest task's by 0.18 with a Sup-21K backbone and 0.73 with MAE -- while the random-projection head, one ridge solution over every class, shows 0.001. Since a task is newest at the stage where a_i(i) is measured and oldest at the stage where a_i(T) is, removing the bias raises the second far more than the first, which is the shape Forgetting rewards: at 0.5 on MAE the old tasks gain 2.33 and the new ones lose 0.33")
    subparsers.add_argument('--rp_task_debias_mode', default='mean', type=str, choices=['mean', 'std', 'both'], help="which half of the task-recency bias to remove. The survey places it in both the bias terms and the weight norms, and Weight Aligning corrects the norms multiplicatively; measured here the multiplicative half is already absorbed by the per-sample standardisation the fusion applies, so 'std' alone moves nothing on MAE (47.81 against 47.78) and hurts Sup-21K as it grows. 'mean' removes the offset, 'both' is BiC's per-task affine form with its parameters read off the sample instead of learned from a validation set")
    ...
    subparsers.add_argument('--rp_class_fusion_gate', default='none', type=str, choices=['none', 'margin', 'margin_both', 'entropy', 'relative'], help="scale the class-fusion weight per sample by how undecided the routed head is; 'margin' uses its normalised top-2 margin, 'margin_both' additionally scales by the RP head's own margin so the blend opens only when the routed head is undecided AND the RP head is decided, 'entropy' uses normalised entropy, 'relative' compares the two heads' margins instead of thresholding the routed head's, 'none' fuses every sample equally")
