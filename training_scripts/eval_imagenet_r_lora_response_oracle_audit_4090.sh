...
LOG_PATH="${OUTPUT_ROOT}/${RUN_BASENAME}_eval_lora_response_oracle_audit_p${PRIOR_TAG}_t${TEMP_TAG}.log"
if [[ -s "${LOG_PATH}" ]]; then
...
cd "${REPO_ROOT}"
echo "Full-rank LoRA response audit: TII ranking vs normalized K/V response vs fixed 2+2 union"
echo "Run=${RUN_DIR}; rank=${LORA_RANK}; TII prior=${TII_PRIOR_WEIGHT}; temperature=${LOGIT_TEMPERATURE}"
echo "Audit only: exhaustive predictions stay unchanged and no router is trained."
...
PYTHONUNBUFFERED=1 "${PYTHON_BIN}" -m torch.distributed.run \
  ...
  --progressive_lora_response_audit \
...
END_TIME="$(date +%s)"
printf 'LoRA response oracle audit wall time seconds: %s\n' "$((END_TIME - START_TIME))" | tee -a "${LOG_PATH}"
FINAL_LINE="$(grep "Average accuracy till task10" "${LOG_PATH}" | tail -n 1 || true)"
echo "Final full-rank LoRA response audit metrics:"
echo "${FINAL_LINE}"
...
tii = metric("WinnerRecall@4")
union = metric("ResponseUnionRecall@2x2")
cost = metric("ResponseUnionLoRA/sample")
gain = union - tii
print(f"Full-rank response union gain over TII top-4: {gain:+.4f} points")
passed = gain >= 3.0 and cost <= 4.0001
print("LORA_RESPONSE_AUDIT_GATE=" + ("PASS" if passed else "FAIL"))
' "${FINAL_LINE}"
