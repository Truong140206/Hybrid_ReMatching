...
RUN_NAME="${RUN_NAME_OVERRIDE:-imr_lora_cfs_matched_budget_10tasks_seed${SEED}}"
...
# Fair CFS ablation for ImageNet-R:
# - legacy CTIRD, as in the baseline;
# - CFS diversity sampling only;
# - no full replay and no boundary replay, so CRCT keeps the baseline update count.
