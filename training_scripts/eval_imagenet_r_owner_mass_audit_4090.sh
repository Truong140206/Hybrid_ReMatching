...
LOG_PATH="${OUTPUT_ROOT}/$(basename "${RUN_DIR}")_eval_owner_mass_consensus_cap5_i2_c5_dev.log"
...
if [[ -s "${LOG_PATH}" ]]; then
  echo "Refusing to overwrite owner-mass audit log: ${LOG_PATH}" >&2
  exit 3
fi
...
echo "Owner-mass rescue: preserve certified consensus and score only ambiguous evaluated adapters."
echo "Score = log P(class belongs to adapter owner task | all seen classes); no fitted threshold."
echo "Only an ambiguous task disagreement is rerouted; within-task ranking and the TII tail are preserved."
...
PYTHONUNBUFFERED=1 "${PYTHON_BIN}" -m torch.distributed.run \
  ...
  --progressive_prediction_owner_mass_audit \
...
END_TIME="$(date +%s)"
printf 'Owner-mass audit wall time seconds: %s\n' \
...
def final_row(path):
    ...
    if not rows:
        raise SystemExit(
            f'OWNER_MASS_ALL_METRIC_GATE=FAIL (missing task-10 row in {path})')
    return rows[-1]
...
def metric(row, name):
    ...
    if not match:
        raise SystemExit(
            f'OWNER_MASS_ALL_METRIC_GATE=FAIL (missing {name})')
...
cost = metric(proposal, 'OwnerMassLoRA/sample')
calls = metric(proposal, 'OwnerMassCalls/sample')
...
print('OWNER_MASS_ALL_METRIC_GATE=' +
