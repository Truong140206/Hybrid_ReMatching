...
# Vectorized exact exhaustive rematching on Split-CIFAR100 (all task LoRAs).
...
if checkpoint.get("real_feature_memory"):
    raise SystemExit("Checkpoint audit failed: real_feature_memory present")
...
echo "Vectorized exhaustive on Split-CIFAR100; rank=${LORA_RANK}; prior=${TII_PRIOR_WEIGHT}; temp=${LOGIT_TEMPERATURE}"
START_TIME="$(date +%s)"
PYTHONUNBUFFERED=1 "${PYTHON_BIN}" -m torch.distributed.run \
  --nproc_per_node=1 --master_port="${MASTER_PORT:-29543}" \
