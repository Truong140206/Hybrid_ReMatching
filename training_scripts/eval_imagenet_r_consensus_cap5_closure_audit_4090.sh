...
LOG_PATH="${OUTPUT_ROOT}/$(basename "${RUN_DIR}")_eval_consensus_cap5_closure_i2_c5_dev.log"
...
echo "Development audit: top-2 adapter consensus anchor with cap-5 fallback"
echo "Locked design: certify only when the TII-top1 adapter wins local task evidence and both initial adapters raw-predict a class owned by the TII-top1 task."
echo "Certified samples stop after the already-evaluated top-2 pair; all others use unchanged one-successor cap-5 closure."
echo "The certificate is threshold-free and label-free; exhaustive agreement remains diagnostic only."
...
END_TIME="$(date +%s)"
printf 'Consensus-cap5 closure audit wall time seconds: %s\n' "$((END_TIME - START_TIME))" | tee -a "${LOG_PATH}"
echo "Final consensus-cap5 closure metrics:"
...
def final_row(path):
    ...
    if not rows:
        raise SystemExit(
            f'CONSENSUS_CAP5_ALL_METRIC_GATE=FAIL '
...
def metric(row, name):
    ...
    if not match:
        raise SystemExit(
            f'CONSENSUS_CAP5_ALL_METRIC_GATE=FAIL (missing {name})')
...
cost = metric(proposal, 'ConsensusClosureLoRA/sample')
calls = metric(proposal, 'ConsensusClosureCalls/sample')
...
for name in (
        'ConsensusClosureWinnerRecall', 'ConsensusClosureExactAgreement',
        'ConsensusClosureCertifiedRate'):
    print(f'  {name}: {metric(proposal, name):.4f}')
print('CONSENSUS_CAP5_ALL_METRIC_GATE=' +
