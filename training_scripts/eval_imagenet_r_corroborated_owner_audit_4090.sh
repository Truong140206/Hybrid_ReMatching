...
LOG_PATH="${OUTPUT_ROOT}/$(basename "${RUN_DIR}")_eval_corroborated_owner_consensus_cap5_i2_c5_dev.log"
...
if [[ -s "${LOG_PATH}" ]]; then
  echo "Refusing to overwrite corroborated-owner audit log: ${LOG_PATH}" >&2
  exit 3
fi
...
echo "Corroborated-owner rescue: preserve consensus except a two-vote self-owner disagreement."
echo "The two-vote condition is the minimum independent corroboration, not a tuned threshold."
echo "Rescued rows use task-local calibrated logits plus the same top-1-safe TII tail."
...
END_TIME="$(date +%s)"
printf 'Corroborated-owner audit wall time seconds: %s\n' \
...
def final_row(path):
    ...
    if not rows:
        raise SystemExit(
            f'CORROBORATED_OWNER_ALL_METRIC_GATE=FAIL (missing task-10 row in {path})')
    return rows[-1]
...
def metric(row, name):
    ...
    if not match:
        raise SystemExit(
            f'CORROBORATED_OWNER_ALL_METRIC_GATE=FAIL (missing {name})')
...
cost = metric(proposal, 'CorroboratedOwnerLoRA/sample')
calls = metric(proposal, 'CorroboratedOwnerCalls/sample')
...
print('CORROBORATED_OWNER_ALL_METRIC_GATE=' +
