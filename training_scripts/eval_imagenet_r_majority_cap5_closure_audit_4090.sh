...
LOG_PATH="${OUTPUT_ROOT}/$(basename "${RUN_DIR}")_eval_majority_cap5_closure_i2_c5_dev.log"
...
echo "Development audit: TII-majority anchor with cap-5 closure for ambiguous samples"
echo "Locked design: if the TII top task owns more than half of seen-class probability mass, evaluate only its LoRA; otherwise use unchanged one-successor cap-5 closure."
echo "The 50% certificate is a mathematical majority, not a fitted threshold; both branches use top-1-safe TII tail completion."
echo "Exhaustive winner/exact agreement is diagnostic only because exhaustive-like routing failed retention on seed 43."
echo "Predeclared gate: all six quality metrics beat conventional; LoRA <5; calls <=4. No old sample, stored feature, calibration, or label is used for routing."
...
PYTHONUNBUFFERED=1 "${PYTHON_BIN}" -m torch.distributed.run \
  ...
  --progressive_prediction_majority_closure_audit \
...
END_TIME="$(date +%s)"
printf 'Majority-cap5 closure audit wall time seconds: %s\n' "$((END_TIME - START_TIME))" | tee -a "${LOG_PATH}"
echo "Final majority-cap5 closure metrics:"
...
def final_row(path):
    ...
    if not rows:
        raise SystemExit(
            f'MAJORITY_CAP5_ALL_METRIC_GATE=FAIL '
...
def metric(row, name):
    ...
    if not match:
        raise SystemExit(
            f'MAJORITY_CAP5_ALL_METRIC_GATE=FAIL (missing {name})')
...
cost = metric(proposal, 'MajorityClosureLoRA/sample')
calls = metric(proposal, 'MajorityClosureCalls/sample')
checks['lora_below_5'] = cost < 5.0
checks['calls_at_most_4'] = calls <= 4.0001
print('Locked efficiency checks:')
for name in ('lora_below_5', 'calls_at_most_4'):
    print(f'  {name}: {"PASS" if checks[name] else "FAIL"}')
print('Diagnostic only (not gates):')
for name in (
        'MajorityClosureWinnerRecall', 'MajorityClosureExactAgreement',
        'MajorityClosureCertifiedRate'):
    print(f'  {name}: {metric(proposal, name):.4f}')
print('MAJORITY_CAP5_ALL_METRIC_GATE=' +
