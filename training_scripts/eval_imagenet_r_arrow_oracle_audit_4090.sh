...
LOG_PATH="${OUTPUT_ROOT}/${RUN_BASENAME}_eval_arrow_oracle_audit_p${PRIOR_TAG}_t${TEMP_TAG}.log"
if [[ -s "${LOG_PATH}" ]]; then
...
cd "${REPO_ROOT}"
echo "Arrow-LoRA audit: TII ranking vs parameter-only Arrow ranking vs their fixed 2+2 union"
...
union = metric("UnionRecall@2x2")
cost = metric("UnionLoRA/sample")
gain = union - tii
print(f"Arrow union gain over TII top-4: {gain:+.4f} points")
passed = gain >= 3.0 and cost <= 4.0001
print("ARROW_AUDIT_GATE=" + ("PASS" if passed else "FAIL"))
' "${FINAL_LINE}"
