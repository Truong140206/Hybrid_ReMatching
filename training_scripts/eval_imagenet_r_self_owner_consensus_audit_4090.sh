...
CONSENSUS_LOG="${OUTPUT_ROOT}/$(basename "${RUN_DIR}")_eval_consensus_cap5_closure_i2_c5_dev.log"
STAGE_DRIFT_REPORT="${OUTPUT_ROOT}/imagenet_r_seed${SEED}_stage_drift.md"
LOG_PATH="${OUTPUT_ROOT}/$(basename "${RUN_DIR}")_eval_self_owner_consensus_cap5_i2_c5_dev.log"
...
for required_path in   "${TRAIN_LOG}"   "${CONVENTIONAL_LOG}"   "${CONSENSUS_LOG}"   "${STAGE_DRIFT_REPORT}"; do
  [[ -s "${required_path}" ]] || {
...
if [[ -s "${LOG_PATH}" ]]; then
  echo "Refusing to overwrite self-owner audit log: ${LOG_PATH}" >&2
  exit 3
fi
...
echo "Self-owner consensus audit: same evaluated adapters and cost as consensus-cap5."
echo "Route key: self-endorsement, then task-vote support, then TII-rank tie break."
echo "Selected self-owner emits its raw seen-class logits; no learned score, threshold, old data, or extra forward."
echo "Locked gate: all six metrics beat conventional; LoRA/calls equal locked consensus and remain below 5/4."
...
PYTHONUNBUFFERED=1 "${PYTHON_BIN}" -m torch.distributed.run \
  ...
  --progressive_prediction_self_owner_audit \
...
END_TIME="$(date +%s)"
printf 'Self-owner consensus audit wall time seconds: %s\n' \
...
def final_row(path):
    ...
    if not rows:
        raise SystemExit(
            f'SELF_OWNER_ALL_METRIC_GATE=FAIL (missing task-10 row in {path})')
    return rows[-1]
...
def metric(row, name):
    ...
    if not match:
        raise SystemExit(
            f'SELF_OWNER_ALL_METRIC_GATE=FAIL (missing {name})')
    return float(match.group(1))


baseline = final_row(sys.argv[1])
consensus = final_row(sys.argv[2])
...
print('Comparison with locked consensus-cap5:')
for name in ('Acc@task', 'Acc@1', 'Acc@5', 'Backward', 'Loss', 'Forgetting'):
    delta = metric(proposal, name) - metric(consensus, name)
    print(f'  {name} delta={delta:+.4f}')

cost = metric(proposal, 'SelfOwnerLoRA/sample')
calls = metric(proposal, 'SelfOwnerCalls/sample')
reference_cost = metric(consensus, 'ConsensusClosureLoRA/sample')
reference_calls = metric(consensus, 'ConsensusClosureCalls/sample')
checks['lora_equal_consensus'] = abs(cost - reference_cost) <= 0.0001
checks['calls_equal_consensus'] = abs(calls - reference_calls) <= 0.0001
checks['lora_below_5'] = cost < 5.0
checks['calls_below_4'] = calls < 4.0
print('Locked efficiency checks:')
for name in (
        'lora_equal_consensus', 'calls_equal_consensus',
        'lora_below_5', 'calls_below_4'):
    print(f'  {name}: {"PASS" if checks[name] else "FAIL"}')

print('Diagnostic only:')
for name in (
        'SelfOwnerWinnerRecall', 'SelfOwnerExactAgreement',
        'SelfOwnerRouteRate', 'SelfOwnerMultipleRate', 'SelfOwnerSupport'):
    print(f'  {name}: {metric(proposal, name):.4f}')

print('SELF_OWNER_ALL_METRIC_GATE=' +
