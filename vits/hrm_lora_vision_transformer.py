    ...
    def forward(self, x,ensemble_id=None,prog=False,id=0, **kwargs):
        ...
        if  ensemble_id!=None:
            ...
            for i in  range(id+1):
                if i <id and id>0:
                    with torch.no_grad():
                        lora = kwargs['lora'](x_ori, depth_id=kwargs['depth_id'], task_id=i, train=kwargs['train'], old=kwargs['old'])['lora_value']
                        qkv = (qkv + lora)
                else:
                        lora = kwargs['lora'](x_ori, depth_id=kwargs['depth_id'], task_id=i, train=kwargs['train'], old=kwargs['old'])['lora_value']
                        qkv = (qkv + lora)
...
class Mlp(nn.Module):
    ...
    def forward(self, x,ensemble_id=None,prog=False,id=0, **kwargs):
        if  ensemble_id!=None:
            ...
            for i in range(ensemble_id.shape[1]):
                task_id = ensemble_id[:,i]
                x = x + 0.4*kwargs['lora'](x, depth_id = kwargs['depth_id'], task_id = task_id, train=kwargs['train'], position='fc1', old=kwargs['old'])['lora_value']
            x = self.act(x)
            x = self.drop1(x)
            x = self.fc2(x)
            for i in range(ensemble_id.shape[1]):
                task_id = ensemble_id[:,i]
                x = x + 0.4*kwargs['lora'](x, depth_id = kwargs['depth_id'], task_id = task_id, train=kwargs['train'], position='fc2', old=kwargs['old'])['lora_value']      
        elif prog:
            x_ori=x
            # x = self.fc1(x)
            ...
            for i in range(id+1):
                if i ==0:
                    with torch.no_grad():
                        x = self.fc1(x) + kwargs['lora'](x, depth_id = kwargs['depth_id'], task_id = i, train=kwargs['train'], position='fc1', old=kwargs['old'])['lora_value']
                elif i <id+1:
                    with torch.no_grad():
                        x = x + kwargs['lora'](x_ori, depth_id = kwargs['depth_id'], task_id = i, train=kwargs['train'], position='fc1', old=kwargs['old'])['lora_value'] 
                else:
                    x = x + kwargs['lora'](x_ori, depth_id = kwargs['depth_id'], task_id = i, train=kwargs['train'], position='fc1', old=kwargs['old'])['lora_value']
            x = self.act(x)
            x = self.drop1(x)
            for i in range(id+1):
                if i ==0:
                    with torch.no_grad():
                        x = self.fc2(x) + kwargs['lora'](x, depth_id = kwargs['depth_id'], task_id = i, train=kwargs['train'], position='fc2', old=kwargs['old'])['lora_value']       
                elif i <id+1:
                    with torch.no_grad():
                        x = x + kwargs['lora'](x_ori, depth_id = kwargs['depth_id'], task_id = i, train=kwargs['train'], position='fc2', old=kwargs['old'])['lora_value']                           
                else:
                    x = self.fc2(x) + kwargs['lora'](x_ori, depth_id = kwargs['depth_id'], task_id = i, train=kwargs['train'], position='fc2', old=kwargs['old'])['lora_value']       
...
class Block(nn.Module):
    ...
    def forward(self, x,ensemble_id=None,prog=False,id=0, **kwargs):
        if 'lora' not in kwargs:
            ...
            x = x + self.drop_path1(self.ls1(self.attn(self.norm1(x),ensemble_id,prog,id=id, **kwargs)))
...
class VisionTransformer(nn.Module):
    ...
    def __init__(
        ...
        if lora:
            if lora_type == 'continual':
                self.lora_layer = continual_lora(dim=embed_dim, rank=rank, depth=lora_depth,pool_size=lora_pool_size,)  
            if lora_type == 'hide':
        ...
        #self.mlp1 = MlpMapping(dim=embed_dim, hidden=[2], fc_with_norm=use_fc_norm)
        ...
        #self.head1 = nn.Linear(self.embed_dim, 2)
        if use_mlp_head:
    ...
    def init_weights_proj(self):
        for m in self.mlp_proj:
            ...
            if self.lora_layer is not None and i < self.lora_depth:
                if train:
                    if self.grad_checkpointing and not torch.jit.is_scripting():
                        ...
                        # if i in [0,1]:
                        #     x = self.blocks[i](x, lora=self.lora_layer, task_id=0, depth_id=i, train=train, old=old)
                        # else:
                            x = self.blocks[i](x, prog=prog,id=id,lora=self.lora_layer, task_id=task_id, depth_id=i, train=train, old=old)
                else:
                    if self.grad_checkpointing and not torch.jit.is_scripting():
    ...
    def forward_head(self, res, pre_logits: bool = False, use_mlp_head=False):
        ...
        res['pre_logits'] = x
        res['features'] = x
        # res['pro_features'] = self.l2norm(self.mlp_proj(x))
        if use_mlp_head:
