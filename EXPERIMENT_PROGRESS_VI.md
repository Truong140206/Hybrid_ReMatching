...
M?c tiêu hi?n t?i là c?i ti?n HRM-PET b?ng hai hu?ng l?y ý tu?ng t? paper PMI-CFS:
...
Ðã thêm CFS vào bu?c sinh feature gi? cho CRCT ? c? hai engine:
...
- thêm superclass CIFAR100 d? similarity có ý nghia hon.
...
Ðây là hu?ng ít phá k?t qu? CFS-only hon, vì semantic ch? dóng vai trò di?u ch?nh ph?.
...
Vì v?y hu?ng c?i ti?n ti?p theo là dùng semantic r?t nh?, nhu m?t gate h? tr? CTIRD thay vì t?o target m?i. Ðây là lý do thêm mode `adaptive_gate`.
...
## 6. Semantic projection sát ý tu?ng paper hon

Sau khi th? `semantic_mode=topk_mix` và th?y k?t qu? không vu?t CFS-only, code du?c c?i ti?n thêm m?t hu?ng sát ý tu?ng g?c c?a paper hon: `--semantic_projection`.

Ý tu?ng g?c trong paper:
...
Trong code, phép `R_cd` du?c tri?n khai b?ng m?t phép bi?n d?i tr?c giao ki?u Householder d? không c?n t?o ma tr?n 768 x 768 d?y d?. Ðây là cách nh? hon nhung v?n gi? tinh th?n: dùng quan h? semantic gi?a class d? project feature t? class ngu?n sang class dích.
...
- Ðây là ablation dáng th? ti?p theo. N?u c?i thi?n, có th? trình bày là semantic-aware feature projection dã du?c chuy?n hóa thành semantic-projected CRCT replay trong HRM-PET.
...
- Semantic projection sát ý tu?ng g?c c?a paper hon semantic top-k vì có bu?c project feature t? class ngu?n sang class dích.
...
- Nguyên nhân h?p lý: semantic embedding hi?n t?i v?n là class-name/superclass embedding nh?, chua ph?i CLIP text embedding th?t nhu paper, nên tín hi?u semantic chua d? m?nh d? vu?t CFS-only.
...
Ði?u này sát paper hon vì paper do semantic similarity b?ng cosine similarity gi?a CLIP text features.
